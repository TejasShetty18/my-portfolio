...
import { InteractiveGridPattern } from "@/components/ui/interactive-grid-pattern"; // <--- Import here
...
export function Hero() {
  return (
    <section className="relative py-20 md:py-32 overflow-hidden bg-background">
      ...
      {/* --- 1. THE ANIMATED GRID BACKGROUND --- */}
      <div className="absolute inset-0 w-full h-full pointer-events-none">
        {/* Note: We remove pointer-events-none from the grid itself so interaction works, 
              but keep it on container if needed. For this specific component, 
              we need pointer-events-auto on the SVG to detect hovers. */}
      </div>

      <InteractiveGridPattern
        className={cn(
          "[mask-image:radial-gradient(600px_circle_at_center,white,transparent)]",
          "inset-x-0 inset-y-[-30%] h-[200%] skew-y-12 opacity-50" // Optional: Skew and scale for dynamic look
        )}
        width={40}
        height={40}
        squares={[80, 80]} // High number to ensure coverage on large screens
        squaresClassName="hover:fill-primary/20" // Highlight color on hover
      />

      {/* Decorative Blob (kept from previous design for extra depth) */}
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-6 grid lg:grid-cols-2 gap-12 items-center">
        ...
        <div className="space-y-8 text-center lg:text-left animate-in slide-in-from-bottom-10 fade-in duration-700">
          ...
          <div className="flex justify-center lg:justify-start">
            <Badge variant="outline" className="px-4 py-1.5 text-sm border-primary/20 bg-primary/5 backdrop-blur-sm">
          ...
          <div className="space-y-4">
            ...
            <p className="text-xl md:text-2xl text-slate-600 dark:text-slate-400 font-medium flex flex-col lg:flex-row items-center gap-2 justify-center lg:justify-start">
              ...
              <span className="flex items-center gap-1 text-base bg-secondary/50 px-3 py-1 rounded-full mt-2 lg:mt-0 backdrop-blur-md">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center lg:justify-start items-center pt-4">
            <Button asChild size="lg" className="h-12 px-8 text-base shadow-lg shadow-primary/20 hover:shadow-primary/40 transition-all z-20">
            ...
            <div className="flex items-center gap-3 z-20">
              <Button variant="outline" size="icon" className="h-12 w-12 rounded-full hover:border-primary hover:text-primary transition-colors bg-background/50 backdrop-blur-sm" asChild>
                <Link href={personalInfo.socials.github} target="_blank" aria-label="GitHub">
                  <Github className="h-5 w-5" />
                </Link>
              </Button>
              <Button variant="outline" size="icon" className="h-12 w-12 rounded-full hover:border-blue-600 hover:text-blue-600 transition-colors bg-background/50 backdrop-blur-sm" asChild>
        ...
        <div className="relative mt-8 lg:mt-0 animate-in slide-in-from-right-10 fade-in duration-1000 delay-200 z-10">
          
          {/* Background Gradient Blob */}
          <div className="absolute inset-0 bg-gradient-to-tr from-blue-500/10 to-purple-500/10 rounded-3xl transform rotate-3 scale-105 blur-sm" />

          <Card className="relative bg-slate-950 border-slate-800 shadow-2xl overflow-hidden transform -rotate-1 hover:rotate-0 transition-transform duration-500">
            ...
            {/* Added overflow-x-auto to ensure code doesn't break layout on very small phones */}
            <CardContent className="p-6 font-mono text-sm text-slate-300 overflow-x-auto">
              <div className="space-y-1">
                ...
                {/* Shortened the array slightly for better mobile fit, or keep as is */}
